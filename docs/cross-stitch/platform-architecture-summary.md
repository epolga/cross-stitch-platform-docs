...
| Elastic Beanstalk env name — **live env is `cross-stitch-com-env-clone`** per `.elasticbeanstalk/config.yml`. Uploader's `App.config:17` + `MainWindow.xaml.cs:57` default + `ENVIRONMENT-SETUP.md:12` still carry the stale `cross-stitch-com-env` — active defect on Uploader's Restart-EB button | drift (live vs stale) | `d:/ann/Git/cross-stitch/.elasticbeanstalk/config.yml:1-7` (live), `d:/ann/Git/Uploader/Uploader/App.config:17` (stale), `d:/ann/Git/Uploader/Uploader/MainWindow.xaml.cs:57` (stale), `d:/ann/Git/cross-stitch/ENVIRONMENT-SETUP.md:12` (stale) |
...
| **AWS Elastic Beanstalk** | Uploader (Restart-EB button — currently targets stale name, see drift entry above); cross-stitch (deployment target — live env `cross-stitch-com-env-clone`) | `d:/ann/Git/Uploader/Uploader/Helpers/ElasticBeanstalkHelper.cs`; `d:/ann/Git/cross-stitch/.ebextensions/`, `d:/ann/Git/cross-stitch/.elasticbeanstalk/config.yml` | `d:/ann/Git/cross-stitch/ENVIRONMENT-SETUP.md` (sibling repo — also stale, see drift entry above) |
...
- **Deploy path**: production runs on **AWS Elastic Beanstalk**, `us-east-1`, application `cross-stitch-com`, branch-default environment **`cross-stitch-com-env-clone`** (`d:/ann/Git/cross-stitch/.elasticbeanstalk/config.yml:1-7`) — *confirmed by maintainer as the live env*. Deploy is invoked manually with `eb deploy`; saved EB configs are preserved under `saved_configs/` (`d:/ann/Git/cross-stitch/saved_configs/eb-configuration-2025-12-12.cfg.yml`).
- **Stale references to the old name `cross-stitch-com-env`** that need to be updated:
  - `Uploader/Uploader/App.config:17` — `ElasticBeanstalkEnvironmentName` config key (active defect: Uploader's "Restart EB" button currently targets the wrong env name)
  - `Uploader/Uploader/MainWindow.xaml.cs:57` — hardcoded fallback default if the config key is missing
  - `cross-stitch/ENVIRONMENT-SETUP.md:12` — "Current production environment" line
...
- **EB env rename has already happened and not propagated** — live env is `cross-stitch-com-env-clone` per `.elasticbeanstalk/config.yml`, but Uploader's `App.config:17` and `MainWindow.xaml.cs:57` default still say `cross-stitch-com-env`. The Restart-EB button is therefore broken until those are updated (`d:/ann/Git/Uploader/Uploader/Helpers/ElasticBeanstalkHelper.cs`, `d:/ann/Git/Uploader/Uploader/App.config:17`, `d:/ann/Git/Uploader/Uploader/MainWindow.xaml.cs:57`).
...
| **AWS deployment assumptions** | yes — bucket name `cross-stitch-designs`, sitemap bucket `cross-stitch-sitemap-cache`, CloudFront `d2o1uvvg91z7o4.cloudfront.net`, EB env `cross-stitch-com-env-clone` (live; Uploader's `App.config` still has stale `cross-stitch-com-env`), EB app `cross-stitch-com`, region `us-east-1` (`d:/ann/Git/Uploader/Uploader/App.config`, `d:/ann/Git/cross-stitch/.elasticbeanstalk/config.yml`, `d:/ann/Git/cross-stitch/next.config.js`) | partially — `plan/integration-aws/README.md` is a proposal-only README |
